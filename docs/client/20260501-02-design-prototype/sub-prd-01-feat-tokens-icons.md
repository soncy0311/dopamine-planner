...
- 파일 위치: `docs/client/prototype/index.html`
...
`<style>` 내에 `:root`로 모든 토큰을 정의한다. `docs/client/design-system/tokens/*.css`의 값을 그대로 옮긴다.
...
1. **토큰 값 정확성**: `docs/client/design-system/tokens/*.css`에 정의된 값과 정확히 일치해야 한다. 값을 임의로 변경하지 않는다
