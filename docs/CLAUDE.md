...
├── server/          서버 구현 문서 (Supabase, Next.js API Routes PRD)
└── client/          클라이언트 구현 문서 (디자인 시스템, 프론트엔드 PRD)
```

- 각 영역 아래 **기능 단위 폴더**를 생성한다: `yyyymmdd-nn-{project-name}/`
- 같은 기능의 문서는 shared/server/client 모두 **동일한 `project-name`** 을 사용한다
...
| `main-prd-{name}.md` | server/ | 서버 메인 PRD. 구현 범위·설계(스키마·RLS·API Route 등)를 정의한다 |
| `sub-prd-{name}.md` | server/ | 서버 서브 PRD. 메인 PRD를 세분화한 구현 가이드 |
| `simple-prd-{name}.md` | server/ | 서버 단일 PRD. 소규모 기능의 구현 가이드 |
| `main-prd-{name}.md` | client/ | 클라이언트 메인 PRD. 구현 범위와 설계를 정의한다 |
| `sub-prd-{name}.md` | client/ | 클라이언트 서브 PRD. 메인 PRD를 세분화한 구현 가이드 |
| `simple-prd-{name}.md` | client/ | 클라이언트 단일 PRD. 소규모 기능의 구현 가이드 |
...
3. `server/` — PRD 작성 (서버 구현 설계)
4. `client/` — PRD 작성 (클라이언트 구현 설계)
5. 코드 구현 시작

**각 단계마다 커밋한다.**
서버·클라이언트 PRD는 동일 기능에 대해 모두 작성하는 것이 원칙이며, 한쪽 영역에만 변경이 필요한 기능이라면 해당 영역의 PRD만 작성한다.
...
- PRD 문서는 해당 영역(server/, client/)의 기능 폴더에 위치한다
