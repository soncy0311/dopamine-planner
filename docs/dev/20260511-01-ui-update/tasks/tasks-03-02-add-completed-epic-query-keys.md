...
- [x] 완료 Epic 목록 query key 가 workspace 단위로 생성된다.
- [x] category filter key 는 범위 변경으로 제외되고 grouping helper 에서 구분된다.
- [x] calendar completed count key 가 월 단위로만 생성된다.
- [x] 기존 Epic/category query key 와 namespace 충돌이 없다.

## 실행 기록

- **일시**: 2026-05-12 23:58 KST
- **결과**: `completedEpics`, `calendarCompletedCounts` query key 추가 및 테스트 통과.
