...
- [x] `category_id = null` Epic 이 domain 에서 null category 로 유지된다.
- [x] `"분류 없음"` category row 를 만들지 않는 테스트 또는 검증이 있다.
- [x] web 카드/목록/필터에서 `"분류 없음"` 표시가 검증된다.
- [x] mobile 카드/목록에서 `"분류 없음"` 표시가 검증된다.
- [x] "전체"과 "분류 없음" 필터가 구분된다.
