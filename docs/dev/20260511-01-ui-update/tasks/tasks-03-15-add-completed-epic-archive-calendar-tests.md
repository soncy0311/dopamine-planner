...
3. **UI 테스트**
   - web/mobile 에서 0개, 1~5개, 6개 이상 indicator 규칙을 검증한다.
...
- [ ] 완료 Epic active 복귀 시 archive 목록에서 제외되는 테스트가 있다.
- [ ] 완료 Epic active 복귀 시 calendar count 에서 제외되는 테스트가 있다.
- [ ] `"분류 없음"` group 이 실제 category row 없이 생성되는 테스트가 있다.
- [ ] indicator 0개/1~5개/6개 이상 규칙 테스트가 있다.
- [ ] web/mobile 접근성 label 에 완료 Epic 개수가 반영되는 테스트가 있다.
- [ ] `make lint` 가 통과한다.
- [ ] `make test` 가 통과한다.
