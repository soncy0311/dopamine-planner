...
- [x] 한 달 범위의 완료 Epic count 가 단일 batch 조회로 반환된다.
- [x] `status = 'completed'` 이고 `completed_date` 가 있는 row 만 count 된다.
- [x] active 복귀로 `completed_date = null` 인 row 가 count 에 포함되지 않는다.
- [x] 날짜 셀 렌더링이 개별 Supabase 호출을 유발하지 않는다.

## 실행 기록

- **일시**: 2026-05-12 23:58 KST
- **결과**: 월 단위 count service/hook 추가 및 service 테스트 통과.
