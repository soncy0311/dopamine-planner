...
- fixture 는 다음 케이스를 포함한다.
  ...
  - 분류 삭제 전/후 Epic
  - 같은 날짜 완료 Epic 0개, 1개, 5개, 6개, 10개

### 2. web/mobile 정책 정합

- progress bar 표시 정책이 동일해야 한다.
- "분류 없음" 표시 문구와 필터 의미가 동일해야 한다.
- 완료 Epic archive 의 그룹/날짜 기준이 동일해야 한다.
- 달력 indicator count 규칙이 동일해야 한다.
...
6. 완료 Epic archive 에서 일반 분류와 "분류 없음" 그룹이 모두 표시되는지 확인한다.
7. 달력에서 완료 Epic 개수 0/1/5/6/10 케이스의 indicator 가 규칙과 일치하는지 확인한다.
...
- [ ] Sub-01~03 의 구현 완료 여부와 변경 파일 범위를 확인한다.
- [ ] 공통 QA fixture 를 정의하고 web/mobile 에서 같은 데이터로 테스트할 수 있게 준비한다.
- [ ] progress linear 렌더링 자동 테스트를 web 에 추가 또는 갱신한다.
- [ ] progress accessibility 자동 테스트를 web/mobile 에 추가 또는 갱신한다.
- [ ] category delete detach 동작을 DB 또는 core service 테스트로 검증한다.
- [ ] `categoryId: null` Epic 렌더링 테스트를 web/mobile 에 추가한다.
- [ ] Epic 편집의 분류 제거 및 재지정 흐름을 테스트한다.
- [ ] 완료 Epic archive group mapper 테스트를 추가한다.
- [ ] 완료 Epic active 복귀 제외 테스트를 추가한다.
- [ ] 달력 indicator count 규칙 테스트를 추가한다.
- [ ] Realtime invalidate 가 category/epic/sub 변경에 반응하는지 테스트한다.
- [ ] web 수동 QA 시나리오를 실행하고 결과를 기록한다.
- [ ] mobile 수동 QA 시나리오를 실행하고 결과를 기록한다.
- [ ] `make lint` 를 실행한다.
- [ ] `make test` 를 실행한다.
...
- [ ] `make lint` 통과.
- [ ] `make test` 통과.
- [ ] web 과 mobile 의 Epic progress UI 가 모두 linear 로 통일되어 있다.
- [ ] Sub 0개 Epic 표시 정책이 web/mobile 에서 동일하다.
- [ ] 분류 삭제 후 Epic/Sub 데이터가 유지된다.
- [ ] "분류 없음" Epic 이 카드, 필터, 편집 폼, archive 에서 표시된다.
- [ ] 완료 Epic archive 의 그룹과 날짜 필터 결과가 web/mobile 에서 일치한다.
- [ ] 달력 indicator 규칙이 web/mobile 에서 일치한다.
- [ ] progressbar 와 달력 날짜의 접근성 label 이 요구사항을 충족한다.
- [ ] 기존 Todo/Sub 토글 및 Epic progress 재계산 회귀가 없다.
- [ ] 수동 QA 시나리오 결과가 문서 또는 이슈에 기록되어 있다.
