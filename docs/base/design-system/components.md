...
| 속성 | 설명 |
|------|------|
| **설명** | 단일 fill 형태의 선형 진행률 막대. Epic 진행률 표시의 atomic primitive |
...
|------|------|
| **설명** | 하위 Sub 이슈 완료율을 분절된 segment 로 표현하는 특수/실험용 progress. 각 segment = Sub 1개 와 1:1 대응 |
| **상세 명세** | [`./components/progress-bar.md`](./components/progress-bar.md) |
| **사용 기준** | EpicCard 기본 progress 로 사용하지 않는다. 별도 실험/특수 컴포넌트 정책에서 명시적으로 opt-in 할 때만 사용 |
...
| 구성 | Header (Checkbox + CategoryBadge + PriorityBadge + Title) + ProgressBar + ProgressPercent + ExpandedBody (SubIssueRow[] + "서브 이슈 추가") |
|------|---|
...
| **진행률** | 기본 `ProgressBar` (`linear`) 고정. 산출식 `completedSubCount / totalSubCount`. Sub 0개 시 progress bar 와 percent 를 모두 표시하지 않음 ([`./components/progress-bar.md`](./components/progress-bar.md) §1 참조) |
